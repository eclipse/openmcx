 ...
 * (in C++17), all in a SFINAE-friendly maner.
 *
 * Copyright: 2018 Ricardo Abreu (https://github.com/ricab/)
 * scope_guard is licensed under the The Unlicense.
 * See https://github.com/ricab/scope_guard
 * ---------------------------------------------------------------------------*/
...
namespace sg
{
    ...
    template<typename Callback>
    scope_guard<Callback> make_scope_guard(Callback&& callback)
    noexcept(std::is_nothrow_constructible<Callback, Callback&&>::value); /*
    we need this in the inner namespace due to MSVC bugs preventing
    sg::scope_guard from befriending a sg::make_scope_guard
...
////////////////////////////////////////////////////////////////////////////////
template<typename Callback>
sg::scope_guard<Callback>::~scope_guard() noexcept
{
  if(m_active)
    m_callback();
}
...
////////////////////////////////////////////////////////////////////////////////
template<typename Callback>
inline void sg::scope_guard<Callback>::dismiss() noexcept
{
  m_active = false;
}
...
{
  return scope_guard<Callback>{std::forward<Callback>(callback)};
}
